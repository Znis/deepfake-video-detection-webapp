...
const BodyUploader = ({ email, reload, loadFromHistoryVideos }) => {
  ...
  const fileUploadHandler = async (event) => {
    ...
    try {
      ...
      const response = await axios.post(
        "https://modest-trusted-moose.ngrok-free.app/upload/",
  ...
  let fileName = loadFromHistoryVideos ? loadFromHistoryVideos['Video Name'] : '';
  if (file) {
    fileName = file.name
  }
    ...
    <>
        ...
        <div className="modal-overlay">
          <div className="modal">
            <div className="modal-header">
              <h2>Dark Themed Modal</h2>
              <button className="close-button" onClick={toggleModal}>
                Close
              </button>
            </div>
            <div className="modal-content">
              <p>This is the content of the dark-themed modal.</p>
            </div>
          </div>
        </div>
      )}
      ...
      <div className={classes["body-holder"]}>
       <div className={classes["video-holder"]}>
        {file ? <VideoViewer data={file} /> :  loadFromHistoryVideos ? <VideoViewer data={loadFromHistoryVideos}/> : "" } 
       </div>
