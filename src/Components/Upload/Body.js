...
const Body = (props) => {
  ...
  const reload_data = () => {
    if (props.profile) {
      ...
      axios.post(
          `https://modest-trusted-moose.ngrok-free.app/videos/?email=${props.profile.email}`,
          {
            headers: {
  ...
  const predictVideo = async (path) => {
    try {
      ...
      const response = await axios.post(
        `https://modest-trusted-moose.ngrok-free.app/predict/?path=${path}`,
        {
          headers: {
            "ngrok-skip-browser-warning": "0",
          },
      ...
      setLoadFromHistoryVideos(response.data);
  ...
  const delVideo = async (path) => {
    try {
      setIsLoadingRem(true);
      const response = await axios.post(
        `https://modest-trusted-moose.ngrok-free.app/delete/?path=${path}`,
        {
          headers: {
            "ngrok-skip-browser-warning": "0",
          },
